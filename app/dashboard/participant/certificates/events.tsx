...
const [hoveredIndex, setHoveredIndex] = useState<number | null>(null);
 ...
 const GetEvents = useCallback(async () => {
        try {
            const response = await axios.get("/api/participant/certificates");
    ...
    // In the card component, add this button:
    return (
                                    ...
                                    <Card className="h-full border border-gray-200 dark:border-sky-900/50 rounded-xl bg-white dark:bg-gradient-to-br from-sky-900/80 to-sky-900 transition-all hover:shadow-lg hover:-translate-y-1 p-4">
                                        <div className="space-y-4">
                                            ...
                                            <div className="flex flex-col gap-2 text-sm">
                                                ...
                                                <div className="flex justify-between">
                                                    <span className="text-gray-500 dark:text-sky-400">Event Completed:</span>
                                                    <span className={`${
                                                        event.event.status 
                                                            ? 'text-emerald-600 dark:text-emerald-400' 
                                                            : 'text-amber-600 dark:text-amber-400'
                                                    }`}>
                                                        {event.event.status ? 'Completed' : 'Pending'}
                                                    </span>
                                                </div>
                                                <div className="flex justify-between">
                                                ...
                                                <button
                                                    onClick={() => router.push(`/dashboard/participant/certificates/${event.id}`)}
                                                    disabled={!event.event.Certificatestatus}
